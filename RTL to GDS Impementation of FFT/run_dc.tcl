set top_module fft_8pt
set rtlPath "./"

# Target library
set target_library /home/linux/ieng6/ee260b/public/data/libraries/db/tcbn65gplustc.db
...
# read RTL
analyze -format sverilog -lib WORK fft_variable_bit.v
#analyze -format sverilog -lib WORK cheatsheet.v
...
compile -exact_map -no_design_rule -ungroup_all
...
# Report metric
set period [get_attri [get_clocks clk] period]
set WNS    [get_attri [get_timing_paths] slack]
set area   $AREA
set metric [expr 1.0 / ($area * ($period - $WNS))]
set outFile [open result_dc.rpt w]
puts $outFile "==================================="
puts $outFile "Metric = $metric"
puts $outFile "==================================="
puts $outFile "Area   = $area um^2"
puts $outFile "Period = $period ns"
