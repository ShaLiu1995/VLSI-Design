...
report_timing -unique_pins -slack_lesser_than 1.0 -nworst 10 -sig 6 -path_type full_clock -transition_time -nets -capacitance > top10_worst_paths_wc.rpt

### report end-point timing
### end-points in the design are registers of timing paths and outputs
set endpoints [add_to_collection [all_registers -data_pins] [all_outputs]]
report_timing -slack_lesser_than 1.0 -to $endpoints -max_paths [sizeof_collection $endpoints] -path_type end -nosplit > end_point_timing_wc.rpt
